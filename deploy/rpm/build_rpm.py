...
def _validate_worker_elf_architecture(
    ...
    _validate_elf_bundle(worker_bundle, worker, "Worker")


def _validate_elf_bundle(bundle: Path, entry: Path, description: str) -> None:
    """自检 PyInstaller onedir 产物：入口是 ELF、架构对、软链不出目录。"""
    if bundle.is_symlink():
        raise SystemExit(
            f"PyInstaller {description} bundle must not be a symlink: {bundle}"
        )
    bundle_root = bundle.resolve()
    if entry.is_symlink():
        _validate_bundle_symlink(bundle_root, entry, os.readlink(entry))
    if not _is_elf(entry):
        raise SystemExit(
            f"PyInstaller {description} entry point is not an ELF executable: "
            f"{entry}"
        )
    ...
    for current, directory_names, file_names in os.walk(
        ...
        for file_name in sorted(file_names):
            ...
            headers = _elf_headers(path, f"{description} bundle file {relative}")
            if headers.get("Machine") == "Linux BPF":
                continue
            for field, expected_value in expected.items():
                if headers.get(field) != expected_value:
                    raise SystemExit(
                        f"{description} bundle ELF {relative} has unexpected "
                        f"{field}: {headers.get(field)!r} "
                        f"(expected {expected_value!r})"
                    )


def _validate_deployment_tests(bundle: Path) -> None:
    """验收套件产物：能起来、pytest 在包里、套件源码齐全。"""
    entry = bundle / "neu-box-deployment-tests"
    _require_file(
        entry, "deployment acceptance suite entry point", executable=True,
    )
    _validate_elf_bundle(bundle, entry, "deployment tests")
    observed = _capture(
        [str(entry), "--self-check"],
        f"running deployment acceptance suite self-check: {entry}",
        timeout=60,
    )
    for marker in ("pytest ", "suite: "):
        if marker not in observed:
            raise SystemExit(
                f"deployment acceptance suite self-check output is not "
                f"recognizable (missing {marker!r}): {observed.strip()[:400]}"
            )
...
def _validate_bpf_object(path: Path) -> None:
    ...
    required_sections = {"cgroup/dev", ".maps", ".rodata", "license"}
    ...
    required_symbols = {
        # 程序 + 三个 map + license。devdrv_major / host_mnt_ns 是
        # struct bpf_config 的成员，不是全局符号，不能列在这里。
        "device_reserve",
        "reserved_devices",
        "reserved_majors",
        "container_owner",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--worker-bundle",
        type=_path,
        default=DEFAULT_BUILD / "pyinstaller-dist" / "neuboxd",
    ...
    parser.add_argument(
        "--unit",
        type=_path,
        default=ROOT / "deploy" / "systemd" / "neuboxd.service",
    )
    parser.add_argument(
        "--ctl-bundle",
        type=_path,
        default=DEFAULT_BUILD / "pyinstaller-dist" / "neuboxctl",
        help="prebuilt PyInstaller onedir bundle of the management CLI",
    )
    parser.add_argument(
        "--tests-bundle",
        type=_path,
        default=DEFAULT_BUILD / "pyinstaller-dist" / "neu-box-deployment-tests",
        help="prebuilt PyInstaller onedir bundle of the deployment suite",
    )
    parser.add_argument(
        "--output-dir",
        type=_path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "allow non-repository info/config/unit inputs only for "
            "a development Release ending in '.dev'"
        ),
    )
    return parser
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    repository_inputs = {
        "device info script directory": (
            args.info_dir,
            ROOT / "src" / "neu_box" / "resources" / "info",
        ),
        ...
        "systemd unit": (
            args.unit,
            ROOT / "deploy" / "systemd" / "neuboxd.service",
        ),
    }
    external_inputs = [
        description
    ...
    worker = args.worker_bundle / "neuboxd"
    _require_file(
        worker,
    ...
    _require_dir(args.tests_bundle, "deployment acceptance suite bundle")
    _validate_deployment_tests(args.tests_bundle)
    _require_file(ROOT / "LICENSE", "license")
    _require_file(RPM_DIR / "neuboxd.spec", "RPM spec")
...
def _compose_source(args: argparse.Namespace, topdir: Path) -> tuple[Path, Path]:
    ...
    for script in (rootfs / "usr" / "share" / "neu-box" / "info").glob(
        ...
        script.chmod(0o755)
    # 实机验收套件是 PyInstaller onedir 产物（自带 pytest 与套件源码），
    # 部署机上不需要 Python 环境。
    _copy_tree(
        args.tests_bundle,
        rootfs / "usr" / "libexec" / "neu-box" / "tests",
    )
    tests_entry = (
        rootfs / "usr" / "libexec" / "neu-box" / "tests"
        / "neu-box-deployment-tests"
    )
    ...
    migration_files = sorted(
        path.name for path in (ROOT / "src" / "neu_box" /
                               "migrations").iterdir()
        if re.fullmatch(r"[0-9]{4}_[a-z0-9_]+\.(?:sql|py)", path.name)
    )
    manifest = {
        "component": "neuboxd",
        ...
        "api_version": 2,
        "schema_version": max(
            (int(name[:4]) for name in migration_files), default=0,
        ),
        "migrations": migration_files,
        "upgrade": "pause-dnf-install-setup",
    }
    manifest_path = rootfs / "usr" / "share" / "neu-box" / "manifest.json"
    ...
    spec = specs / "neuboxd.spec"
    spec_template = (RPM_DIR / "neuboxd.spec").read_text(
        encoding="utf-8"
    )
    spec.write_text(
    ...
    with tempfile.TemporaryDirectory(prefix="neu-box-rpmbuild-") as raw_temp:
        ...
        if args.source_only:
            ...
            output_spec = args.output_dir / (
                f"neuboxd-{args.version}-{args.release}.spec"
            )
